-- 090: Fix sponsor bonus_condition parsing - avoid %1% matching "position 10"
-- Champion: use champion/1st/first only; remove broad %1% which incorrectly matched position 10
...
BEGIN
  ...
  -- Step 4: Sponsor bonuses + failure penalties (fix: champion uses champion/1st/first, not %1%)
  FOR v_team IN
  ...
  LOOP
    ...
    IF v_sponsor.bonus_amount IS NOT NULL AND v_sponsor.bonus_amount > 0 AND v_sponsor.bonus_condition IS NOT NULL THEN
      ...
      IF v_sponsor.bonus_condition ILIKE '%position%4%' OR v_sponsor.bonus_condition ILIKE '%top%4%' THEN
        ...
        END IF;
      ELSIF v_sponsor.bonus_condition ILIKE '%champion%' OR v_sponsor.bonus_condition ILIKE '%1st%' OR v_sponsor.bonus_condition ILIKE '%first%' THEN
