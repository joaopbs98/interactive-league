-- 122: Sponsor 2-season duration and removal deduction
-- 1. Add sponsor_signed_at_season to teams
-- 2. end_season: only pay sponsor if S2+ and within 2 seasons; clear when expired
-- 3. RPC for deducting sponsor payment on removal

ALTER TABLE teams ADD COLUMN IF NOT EXISTS sponsor_signed_at_season INTEGER;

COMMENT ON COLUMN teams.sponsor_signed_at_season IS 'Season when sponsor was signed; contract lasts 2 seasons (signed_season and signed_season+1)';

-- RPC: deduct sponsor base payment when team removes sponsor (called from API)
CREATE OR REPLACE FUNCTION deduct_sponsor_on_removal(
  p_team_id UUID,
  p_league_id UUID,
  p_amount INTEGER,
  p_season INTEGER
...
BEGIN
  ...
  PERFORM write_finance_entry(
    ...
    'Sponsor Removal', 'Sponsor contract terminated - deduction season ' || p_season,
    p_season
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Update end_season: sponsor only from S2, 2-season duration, clear when expired
...
BEGIN
  ...
  -- Sponsor payments: only from S2 onwards, and only within 2 seasons of signing
  IF v_season >= 2 THEN
    FOR v_team IN
    ...
    LOOP
      -- Skip if contract expired (signed more than 2 seasons ago)
      ...
      -- Pay only if within 2 seasons (legacy: sponsor_signed_at_season NULL = treat as signed for current season)
      ...
      -- Clear sponsor if just finished 2nd season
      IF v_team.sponsor_signed_at_season IS NOT NULL AND v_season = v_team.sponsor_signed_at_season + 1 THEN
        UPDATE teams SET sponsor_id = NULL, sponsor_signed_at_season = NULL WHERE id = v_team.team_id;
      END IF;
    END LOOP;
