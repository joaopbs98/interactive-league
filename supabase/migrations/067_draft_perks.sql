-- 067: Draft perks - Player of choice (80), Merch %, Upgrade (IL25 spec §4.2)
-- bonus: { type: 'player_choice_80' } | { type: 'merch_pct', value: 10 } | { type: 'upgrade_ticket' }

ALTER TABLE draft_picks ADD COLUMN IF NOT EXISTS bonus JSONB;

COMMENT ON COLUMN draft_picks.bonus IS 'Perk type: player_choice_80 (pick any OVR<=80), merch_pct (value: 1-10), upgrade_ticket';

-- Fix make_draft_pick: use current_owner_team_id for traded picks; validate player_choice_80 OVR <= 80
...
BEGIN
  ...
  -- merch_pct and upgrade_ticket: no player pick - claim without selecting player
  IF v_bonus_type = 'merch_pct' OR v_bonus_type = 'upgrade_ticket' THEN
    UPDATE draft_picks SET is_used = true WHERE id = p_draft_pick_id;
    INSERT INTO draft_selections (draft_pick_id, player_id, item_type) VALUES (p_draft_pick_id, NULL, COALESCE(v_bonus_type, 'unknown'));
    IF NOT EXISTS (SELECT 1 FROM draft_picks WHERE league_id = v_league_id AND season = v_season AND is_used = false) THEN
      UPDATE leagues SET draft_active = false WHERE id = v_league_id;
    END IF;
    PERFORM write_audit_log(v_league_id, 'make_draft_pick', p_actor_user_id,
      json_build_object('pick_id', p_draft_pick_id, 'bonus', v_bonus_type, 'team_id', v_team_id)::jsonb);
    RETURN json_build_object('success', true, 'bonus_type', v_bonus_type, 'team_id', v_team_id);
  END IF;
...
-- Host-only: set bonus on a draft pick (before pick is used)
CREATE OR REPLACE FUNCTION update_draft_pick_bonus(
  p_draft_pick_id UUID,
  p_bonus JSONB,
  p_actor_user_id UUID
)
...
DECLARE
  ...
  v_league_id UUID;
BEGIN
  SELECT dp.id, dp.league_id, dp.is_used
  INTO v_pick
  FROM draft_picks dp
  ...
  IF v_pick.is_used THEN
    RETURN json_build_object('success', false, 'error', 'Cannot change bonus on used pick');
  END IF;

  v_league_id := v_pick.league_id;

  -- Verify actor is league commissioner
  ...
  UPDATE draft_picks SET bonus = p_bonus WHERE id = p_draft_pick_id;

  RETURN json_build_object('success', true, 'bonus', p_bonus);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
