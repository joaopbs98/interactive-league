-- 072: Add merchandise revenue payout to end_season (top 14, IR+position, 30% base)
-- Teams get 30% base + merch_percentage (draft bonuses). Only top 14 players by OVR contribute.
...
BEGIN
  ...
  FOR v_row IN
    ...
    LIMIT 14
  LOOP
    v_ir := LEAST(5, GREATEST(1, COALESCE((v_row.ir)::INTEGER, 1)));
    v_ir_val := CASE v_ir
      WHEN 1 THEN 100000
      WHEN 2 THEN 250000
      WHEN 3 THEN 500000
    ...
    v_pos_mult := CASE
      WHEN v_first_pos IN ('GK','CB','LB','RB','LWB','RWB','CDM') THEN 0.9
      WHEN v_first_pos IN ('LW','RW','ST','CF','CAM') THEN 1.2
      ELSE 1.0
    END;

    v_sum := v_sum + (v_ir_val * v_pos_mult);
  END LOOP;
...
-- Update end_season: add Step 8b Merchandise revenue (after wages, before CompIndex)
