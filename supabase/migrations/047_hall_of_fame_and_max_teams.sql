-- 047: Hall of Fame table, max_teams, and HOF points in end_season

-- Hall of Fame: tracks points per team per season
CREATE TABLE IF NOT EXISTS hall_of_fame (
  ...
  hof_points INTEGER NOT NULL,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  UNIQUE(league_id, team_id, season)
);

CREATE INDEX IF NOT EXISTS idx_hof_league_season ON hall_of_fame(league_id, season);
CREATE INDEX IF NOT EXISTS idx_hof_team ON hall_of_fame(league_id, team_id);

ALTER TABLE hall_of_fame ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view HOF in their leagues" ON hall_of_fame
  FOR SELECT USING (
    league_id IN (SELECT league_id FROM teams WHERE user_id = auth.uid())
  );

-- Configurable max teams per league
ALTER TABLE leagues ADD COLUMN IF NOT EXISTS max_teams INTEGER DEFAULT 20;

-- Update end_season to award HOF points (linear: 1st=100, 2nd=80, 3rd=60, 4th=40, 5th=20, 6th+=10)
...
BEGIN
  ...
  LOOP
    ...
    -- HOF points: linear 100, 80, 60, 40, 20, 10...
