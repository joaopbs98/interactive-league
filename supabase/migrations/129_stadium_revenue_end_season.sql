-- 129: Stadium revenue payout at end_season (IL25 spec)
-- Matchday revenue paid for the season that just ended.
-- Uses capacity, visitor_focus, seasonal_performance, standings.played.

CREATE OR REPLACE FUNCTION stadium_attendance_pg(
  p_capacity INTEGER,
  p_visitor_focus TEXT,
  p_seasonal_performance TEXT
) RETURNS INTEGER AS $$
DECLARE
  v_p INTEGER;
BEGIN
  IF p_seasonal_performance IS NULL OR p_seasonal_performance = '' THEN
    RETURN 0;
  END IF;

  v_p := CASE p_seasonal_performance
  ...
  RETURN CASE p_visitor_focus
    WHEN 'Core Fanbase' THEN GREATEST(36000, p_capacity - (p_capacity - 36000) * ((10 - v_p)::NUMERIC / 9))
    WHEN 'Local Casuals' THEN (p_capacity * (0.4 + 0.05 * v_p))::INTEGER
    WHEN 'Tourists' THEN (p_capacity * (0.2 + 0.08 * v_p))::INTEGER
    WHEN 'Hospitality & VIP' THEN
      CASE WHEN p_seasonal_performance = 'UCL Winners' THEN p_capacity
           ELSE (p_capacity * 0.08 * v_p)::INTEGER END
    ELSE 0
  END;
END;
$$ LANGUAGE plpgsql IMMUTABLE;

CREATE OR REPLACE FUNCTION stadium_revenue_pg(
...
DECLARE
  ...
  v_raw NUMERIC;
BEGIN
  v_price := CASE p_visitor_focus
    WHEN 'Core Fanbase' THEN 66.86
    WHEN 'Local Casuals' THEN 97.35
    WHEN 'Tourists' THEN 101.35
    WHEN 'Hospitality & VIP' THEN 130.02
    ELSE 0
  END;

  v_raw := p_attendance * v_price * (p_games_played::NUMERIC / 2);
  RETURN (ROUND(v_raw / 100000) * 100000)::INTEGER;
END;
$$ LANGUAGE plpgsql IMMUTABLE;

-- Replace end_season to add stadium revenue payout (after merchandise, before comp_index)
