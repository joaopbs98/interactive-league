-- Add expendables section to teams table
...
-- Create index for pack purchases
...
BEGIN
  ...
  -- If squad has less than 23 players, move to squad (IL25 spec)
...
BEGIN
  ...
  -- If squad is full (23 players), move to expendables (IL25 spec)
  IF squad_size >= 23 THEN
    -- Get current expendables
    SELECT COALESCE(expendables, '{}') INTO current_expendables
    FROM teams
