-- 135: Fix end_season() releasing re-signed players
-- The "release players whose contract expired" step matched player_id against
-- ANY expired contract anywhere in the league, not the specific team's contract.
-- A player released by Team A in a past season and later signed by Team B (same
-- league) with a healthy active contract would get wrongly un-rostered from
-- Team B every subsequent end_season, because their old expired contract at
-- Team A still satisfied "player_id IN (expired contracts in this league)".
-- Fix: join the expired-contract check to the SAME team_id as the player's
-- current row, not the whole league.
...
DECLARE
  ...
  v_stadium_total INTEGER := 0;
  v_merch_rev INTEGER;
  v_merch_pct NUMERIC;
  v_stadium_rev INTEGER;
...
BEGIN
  ...
  IF v_season >= 2 THEN
    ...
    LOOP
      ...
      v_already_paid := EXISTS (SELECT 1 FROM finances WHERE team_id = v_team.team_id AND league_id = p_league_id AND season = v_season AND reason = 'Sponsor Payment');
      ...
      IF v_term.payout_type = 'performance_tier' THEN
        ...
        IF v_comp IS NOT NULL AND v_stage_pattern IS NOT NULL THEN
          ...
          IF v_tier IS NOT NULL THEN
            ...
            END IF;
          ELSE
            IF NOT v_already_paid AND v_term.base_payment > 0 THEN
              PERFORM write_finance_entry(
  ...
  LOOP
    ...
    ELSIF v_obj.trigger_condition ILIKE '%champion%' AND v_team_position = 1 THEN
      ...
      PERFORM write_finance_entry(v_obj.to_team_id, p_league_id, v_obj.reward_amount,
        'Trade Objective', v_obj.description, v_season);
      UPDATE objectives SET fulfilled = true WHERE id = v_obj.id;
    ELSIF v_obj.trigger_condition ILIKE '%relegat%' AND v_team_position IS NOT NULL THEN
      -- "avoid relegation": fulfilled if team finished outside the bottom 3
      IF v_team_position <= (SELECT COUNT(*) FROM standings WHERE league_id = p_league_id AND season = v_season) - 3 THEN
        PERFORM write_finance_entry(v_obj.from_team_id, p_league_id, -v_obj.reward_amount,
          'Trade Objective', v_obj.description, v_season);
        PERFORM write_finance_entry(v_obj.to_team_id, p_league_id, v_obj.reward_amount,
          'Trade Objective', v_obj.description, v_season);
  ...
  -- Fixed: only release a player from the SAME team whose contract with them
  -- just expired, not from any team if they have an expired contract anywhere
  -- else in the league (see migration header).
  UPDATE league_players lp SET team_id = NULL
  FROM contracts c
  WHERE lp.team_id = c.team_id
    AND lp.player_id = c.player_id
    AND lp.team_id IN (SELECT id FROM teams WHERE league_id = p_league_id)
  ...
  -- Stadium revenue (IL25): capacity, visitor_focus, seasonal_performance, standings.played
  FOR v_team IN
    SELECT t.id as team_id, t.capacity, t.visitor_focus, t.seasonal_performance
    FROM teams t WHERE t.league_id = p_league_id
  LOOP
    SELECT COALESCE(s.played, 0) INTO v_games_played
    FROM standings s WHERE s.league_id = p_league_id AND s.season = v_season AND s.team_id = v_team.team_id;

    IF COALESCE(v_team.capacity, 0) > 0 AND v_team.visitor_focus IS NOT NULL AND v_team.seasonal_performance IS NOT NULL AND v_games_played > 0 THEN
      v_attendance := stadium_attendance_pg(COALESCE(v_team.capacity, 0), v_team.visitor_focus, v_team.seasonal_performance);
      v_stadium_rev := stadium_revenue_pg(v_attendance, v_team.visitor_focus, v_games_played);
      IF v_stadium_rev > 0 THEN
        PERFORM write_finance_entry(
          v_team.team_id, p_league_id, v_stadium_rev,
          'Stadium', 'Season ' || v_season || ' matchday revenue',
          v_season
        );
        v_stadium_total := v_stadium_total + v_stadium_rev;
      END IF;
    END IF;
  END LOOP;
  ...
  -- Recompute stock values now that comp_index, wages, loans and merch are up to date
  PERFORM update_league_stock_prices(p_league_id);
