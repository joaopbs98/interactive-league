-- 123: Competition prize money (UCL/UEL/UECL/Super Cup) from S2 onwards; skip all prize money for S1

-- Helper: get prize money amounts by season and competition (e.g. 55000000 = $55M)
CREATE OR REPLACE FUNCTION get_competition_prize(p_season INTEGER, p_comp TEXT, p_bonus_type TEXT)
RETURNS INTEGER AS $$
DECLARE
  v_start INTEGER;
  v_stage INTEGER;
...
BEGIN
  ...
  IF p_bonus_type = 'supercup' THEN
    RETURN CASE p_season
      WHEN 2 THEN 5000000 WHEN 3 THEN 7500000 WHEN 4 THEN 10000000 WHEN 5 THEN 12500000
      WHEN 6 THEN 15000000 WHEN 7 THEN 20000000 WHEN 8 THEN 25000000 WHEN 9 THEN 30000000
      ELSE 30000000
    END;
  END IF;
  ...
  CASE p_comp
    WHEN 'ucl' THEN
      v_start := CASE p_season
        WHEN 2 THEN 55000000 WHEN 3 THEN 72500000 WHEN 4 THEN 92500000 WHEN 5 THEN 115000000
        WHEN 6 THEN 140000000 WHEN 7 THEN 170000000 WHEN 8 THEN 205000000 WHEN 9 THEN 245000000
        ELSE 245000000
      END;
      v_stage := CASE p_season
        WHEN 2 THEN 9000000 WHEN 3 THEN 12500000 WHEN 4 THEN 15000000 WHEN 5 THEN 17500000
        WHEN 6 THEN 20000000 WHEN 7 THEN 25000000 WHEN 8 THEN 30000000 WHEN 9 THEN 35000000
        ELSE 35000000
      END;
    WHEN 'uel' THEN
      v_start := CASE p_season
        WHEN 2 THEN 45000000 WHEN 3 THEN 60000000 WHEN 4 THEN 80000000 WHEN 5 THEN 97500000
        WHEN 6 THEN 120000000 WHEN 7 THEN 145000000 WHEN 8 THEN 175000000 WHEN 9 THEN 210000000
        ELSE 210000000
      END;
      v_stage := CASE p_season
        WHEN 2 THEN 7500000 WHEN 3 THEN 10000000 WHEN 4 THEN 12500000 WHEN 5 THEN 15000000
        WHEN 6 THEN 17500000 WHEN 7 THEN 20000000 WHEN 8 THEN 22500000 WHEN 9 THEN 27500000
        ELSE 27500000
      END;
    WHEN 'uecl' THEN
      v_start := CASE p_season
        WHEN 2 THEN 40000000 WHEN 3 THEN 50000000 WHEN 4 THEN 40000000 WHEN 5 THEN 82500000
        WHEN 6 THEN 100000000 WHEN 7 THEN 120000000 WHEN 8 THEN 145000000 WHEN 9 THEN 175000000
        ELSE 175000000
      END;
      v_stage := CASE p_season
        WHEN 2 THEN 6000000 WHEN 3 THEN 7500000 WHEN 4 THEN 6000000 WHEN 5 THEN 12500000
        WHEN 6 THEN 15000000 WHEN 7 THEN 17500000 WHEN 8 THEN 20000000 WHEN 9 THEN 22500000
        ELSE 22500000
      END;
    ELSE
      RETURN 0;
  ...
  IF p_bonus_type = 'start' THEN
    RETURN v_start;
  ELSIF p_bonus_type = 'stage' THEN
    RETURN v_stage;
  ELSE
    RETURN 0;
  END IF;
...
-- Update end_season: S1 no prize money; S2+ domestic + competition + supercup
...
BEGIN
  ...
  -- Domestic prize money + HOF: prize only from S2, HOF always
  FOR v_team IN
  ...
  IF v_season >= 2 THEN
    ...
    -- Competition prize money (UCL/UEL/UECL) from team_competition_results
    -- Start bonus for all; semi/final/win bonuses cumulative by stage
    FOR v_tcr IN
  ...
  -- Sponsor payments: only from S2 onwards, 2-season duration (from migration 122)
  IF v_season >= 2 THEN
    FOR v_team IN
      SELECT t.id as team_id, t.sponsor_id, t.sponsor_signed_at_season, t.merch_percentage
      FROM teams t
      WHERE t.league_id = p_league_id AND t.sponsor_id IS NOT NULL
    LOOP
      IF v_team.sponsor_signed_at_season IS NOT NULL AND v_season > v_team.sponsor_signed_at_season + 1 THEN
      ...
      IF v_team.sponsor_signed_at_season IS NULL OR v_season <= v_team.sponsor_signed_at_season + 1 THEN
        PERFORM write_finance_entry(
        ...
        IF v_sponsor.bonus_amount IS NOT NULL AND v_sponsor.bonus_amount > 0 AND v_sponsor.bonus_condition IS NOT NULL THEN
          IF eval_sponsor_bonus(v_team.team_id, p_league_id, v_season, v_sponsor.bonus_condition) THEN
            v_bonus_met := true;
            PERFORM write_finance_entry(
