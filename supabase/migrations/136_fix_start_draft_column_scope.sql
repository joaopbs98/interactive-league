-- 136: Fix start_draft() — always threw "column points does not exist"
-- The inner subquery only SELECTed team_id, but the outer ARRAY_AGG's ORDER BY
-- referenced points/goal_diff/goals_for, which were never carried out of the
-- subquery's result set. This meant start_draft() could never succeed — every
-- call errored before a single draft pick was ever created.
...
BEGIN
  ...
  -- Fixed: the subquery must SELECT the sort columns too, not just team_id, or the outer
  -- ARRAY_AGG's ORDER BY has nothing to reference.
