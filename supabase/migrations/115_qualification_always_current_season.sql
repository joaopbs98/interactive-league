-- 115: Qualification always uses current season domestic standings (not previous)
-- Flow: domestic plays first within the season, then international groups are populated from those standings.
-- Super Cup remains: UCL vs UEL winners from previous season (S2+).
...
DECLARE
  ...
  v_n INTEGER;
BEGIN
  -- Always use current season standings for qualification (domestic plays first, then international)
  v_qual_season := p_season;

  -- Get top 14 teams from current season standings
  ...
  IF v_teams IS NULL OR array_length(v_teams, 1) < 6 THEN
    RETURN json_build_object('success', false, 'error', 'Need at least 6 teams in standings for qualification. Ensure domestic rounds are finished.');
  END IF;
