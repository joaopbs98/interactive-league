-- Migration 018: Fix starter squad to use player table directly
-- This migration updates the generate_starter_squad function to use the player table directly
-- instead of relying on league_players table which may contain artificial players

-- Drop the existing function
DROP FUNCTION IF EXISTS generate_starter_squad(UUID, UUID);

-- Create the updated function that uses player table directly
...
BEGIN
  ...
  result := json_build_object(
    'success', true,
    'message', 'Starter squad generated successfully with real players',
