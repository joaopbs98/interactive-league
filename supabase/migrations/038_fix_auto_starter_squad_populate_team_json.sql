-- 038: Fix auto_starter_squad to also populate teams.squad JSON
-- so that dashboard/tactics pages can render the players.
...
BEGIN
  ...
  LOOP
    ...
    v_squad_json := v_squad_json || jsonb_build_object(
      ...
      'name', COALESCE(v_player.full_name, v_player.name),
      'positions', v_player.positions,
      'overall_rating', COALESCE(v_player.overall_rating, 60),
      'image', v_player.image,
      'club_name', v_player.club_name,
      'country_name', v_player.country_name
    );
  ...
  -- Write the squad JSON to the teams table
