-- Migration 019: Fix starter squad to use complete player data
-- This migration updates the generate_starter_squad function to use complete player data
-- from the player table, similar to how league creation works

-- Drop the existing function
DROP FUNCTION IF EXISTS generate_starter_squad(UUID, UUID);

-- Create the updated function that uses complete player data
...
BEGIN
  ...
  -- Update team with complete player data (like league creation does)
  UPDATE teams 
  SET squad = (
  ...
  result := json_build_object(
    'success', true,
    'message', 'Starter squad generated successfully with complete player data',
