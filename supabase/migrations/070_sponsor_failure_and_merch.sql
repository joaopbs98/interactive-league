-- 070: Sponsor failure clauses + merchandise revenue (IL25 Phase 11)

-- Extend sponsors with failure clauses
ALTER TABLE sponsors ADD COLUMN IF NOT EXISTS transfer_request_count INTEGER DEFAULT 0;
ALTER TABLE sponsors ADD COLUMN IF NOT EXISTS merch_modifier NUMERIC DEFAULT 0;
ALTER TABLE sponsors ADD COLUMN IF NOT EXISTS repayment_penalty INTEGER DEFAULT 0;

COMMENT ON COLUMN sponsors.transfer_request_count IS 'On failure: set transfer_request on N highest-rated players';
COMMENT ON COLUMN sponsors.merch_modifier IS 'On failure: add to merch % (e.g. -2.5)';
COMMENT ON COLUMN sponsors.repayment_penalty IS 'On failure: add repayment to finances (e.g. 15000000)';

-- Teams: merch percentage for revenue calculation
ALTER TABLE teams ADD COLUMN IF NOT EXISTS merch_percentage NUMERIC DEFAULT 0;
ALTER TABLE teams ADD COLUMN IF NOT EXISTS merch_base_revenue INTEGER DEFAULT 0;

COMMENT ON COLUMN teams.merch_percentage IS 'Merchandise % (modified by sponsor failure)';
COMMENT ON COLUMN teams.merch_base_revenue IS 'Base merchandise revenue for calculation';

-- Update end_season Step 4: on sponsor failure, apply transfer_request, merch_modifier, repayment
-- Injects failure logic after sponsor bonus check
...
BEGIN
  ...
  LOOP
    ...
    -- Failure penalties: transfer_request, merch_modifier, repayment
